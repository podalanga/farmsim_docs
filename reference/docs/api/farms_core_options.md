...
**Note**: `Options` is a `dict` subclass, not a frozen dataclass. It provides attribute-style access via `__getattr__` and `__setattr__` that delegate to dict operations.
...
| `gains` | list[float] | - | Kp/Kd/Kv gains (3-element list) |
