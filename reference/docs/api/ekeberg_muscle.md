...
| `gain` | `float64[N_joints_total]` | Transform gain per joint |
| `bias` | `float64[N_joints_total]` | Transform bias per joint |
...
| `torques_implicit(iteration)` | Return `JOINT_CMD_TORQUE` values (after `step`) |
| `springrefs(iteration)` | Return transformed joint offsets |
