...
!!! note "Source Files"
    - `experiments/zbot_bout_glide/controller/` — Example Zbot controller implementations
...
`AnimatController` is a base class in `farms_core.model.control`. It ships with no-op default implementations of `from_options()`, `positions()`, and `torques()` (which return zero commands), so you override the ones you need for useful behaviour. For a custom controller you typically override:
...
class ZbotSineCPG(AnimatController):
    ...
    def from_options(cls, config, experiment_options, animat_i, animat_data, animat_options):
        ...
        animat_data : AmphibiousData
            ...
            Sensor arrays live under `animat_data.sensors` (e.g. `.sensors.joints`).
        animat_options : AmphibiousOptions
            Full animat configuration loaded from animat_config.yaml. Joint names,
            control types, and torque limits are read from `animat_options.control.motors`.
        """
        # Build the 7-tuple of joint-name lists (one entry per ControlType)
        # plus the matching max_torque tuple, from the motor definitions.
        # NOTE: `joints_names` MUST be a 7-tuple (len == len(ControlType)); passing
        # a flat list of joint names will fail the base-class assertion.
        motors = animat_options.control.motors
        all_joints = [m.joint_name for m in motors]
        control_types = {
        ...
        return cls(
            ...
            max_torques=cls.max_torques_from_control_types(
                joints_names=all_joints,
                max_torques={m.joint_name: m.limits_torque[1] for m in motors},
                joints_control_types=control_types,
            ),
...
| Robot does nothing useful | `from_options` not overridden | The base class provides a no-op `from_options` that builds empty joint lists; override it to wire up real joints/controls |
...
- [`AnimatController` API](../reference/core/core-control.md) — full method signatures
- [Mathematical Models](../explanation/mathematical-models.md) — CPG ODE and Ekeberg equations
- [`AmphibiousController` API](../reference/amphibious/amphibious-controller.md) — production CPG
- [CPG Oscillator Network API](../reference/amphibious/cpg-oscillators.md) — oscillator data structures
- [Ekeberg Muscle Model API](../reference/amphibious/ekeberg-muscle.md) — muscle torque computation
- [Swimming Experiment](zbot-experiment.md) — full YAML reference
