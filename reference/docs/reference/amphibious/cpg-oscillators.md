...
The FARMS CPG supports **three sensory feedback pathways** (implemented as `ode_stretch`, `ode_contacts`, and `ode_xfrc`) that modulate the oscillator dynamics. These add to the `dstate` buffer computed above.
...
class NetworkODE(AnimatNetwork):
    def __init__(self, data, integrator='dopri5', **kwargs):
        self.modulo = kwargs.pop('modulo', 1)
        super().__init__(data=data, n_iterations=np.shape(data.state.array)[0])
        self.dstate = np.zeros_like(data.state.array[0, :])
        self.ode = kwargs.pop('ode', ode_oscillators_sparse)  # derivative kernel
        self.integrator = integrator
        self.integrator_kwargs = kwargs            # e.g. nsteps, max_step, verbosity
        self.solver = scipy.integrate.ode(f=self.ode)  # ode_oscillators_sparse
        self.initialize_episode()                  # calls solver.set_integrator(...)
```
...
!!! note "Integrator kwargs are forwarded verbatim"
    ...
    network with `integrator='dopri5', nsteps=1000, max_step=<physics timestep>,
    verbosity=3`.
...
def step(self, iteration, time, timestep, checks=False, strict=False):
    ...
    if iteration % self.modulo:
        # Sub-step: copy previous state (no integration)
        ...
        if not self.solver.successful():
            if strict:
                ...
                self.solver.set_initial_value(y=self.solver.y, t=time+timestep)  # Reset
    # Handle drive (runs on EVERY iteration, including modulo-skipped ones)
...
| 2 | `POS2FREQ` | Joint position → frequency | Not implemented in `ode.pyx` |
| 3 | `VEL2FREQ` | Joint velocity → frequency | Not implemented in `ode.pyx` |
| 4 | `TOR2FREQ` | Joint torque → frequency | Not implemented in `ode.pyx` |
| 5 | `POS2AMP` | Joint position → amplitude | Not implemented in `ode.pyx` |
| 6 | `VEL2AMP` | Joint velocity → amplitude | Not implemented in `ode.pyx` |
| 7 | `TOR2AMP` | Joint torque → amplitude | Not implemented in `ode.pyx` |
...
| 13 | `REACTION2AMP` | Contact force magnitude → amplitude | Not implemented in `ode_contacts` |
