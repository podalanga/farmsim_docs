...
!!! warning "Unverified"
    "Tegotae phase feedback" is listed here but no Tegotae-specific kwargs exist in `defaults_from_convention`. Tegotae connection types (`STRETCH2TEGOTAE`, `REACTION2TEGOTAE`, `LATERAL2TEGOTAE`) exist in the `ConnectionType` enum and are dispatched in `ode.pyx`, but they are not part of the default connectivity generation. They must be configured explicitly in the connectivity arrays (`osc2osc`, `joint2osc`, `contact2osc`, `xfrc2osc`).
