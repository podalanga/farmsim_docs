...
!!! bug "Confirmed: `IntegrationException` is never defined"
    `NetworkODE.step()` does `raise IntegrationException(message)` on the
    ...
    [`farms-sim.md`](../sim/farms-sim.md) for the sibling instance of this
    pattern elsewhere in the codebase. Until fixed upstream, treat
    `strict=True` as effectively "crash with `NameError` on failure" rather
    than "raise a documented exception type."
