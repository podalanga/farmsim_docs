...
The `position_cmds(iteration)` method (defined in `JointsControlCy`, the base of `JointsMusclesCy`) reads `JOINT_CMD_POSITION` from the array and returns it as a numpy array.
...
cpdef void step(self, unsigned int iteration):
    ...
    for joint_i in range(self.n_joints):
        ...
        pos = (  # Transform current position into convention space
        ...
        if amplitudes[osc_i_0] < self.threshold:  # Swimming (low amplitude)
...
!!! note "Position is transformed into convention space first"
    Unlike `PositionMuscleCy`, which reads `outputs`/`offsets` directly, `PositionPhaseCy`
    first maps the sensed joint position back into convention space via
    `pos = (pos_raw - transform_bias)/transform_gain`, so that `dif` is computed in the
    same space as `phases` and `offsets`. The offset `δ_j` is folded into `desired_angle`
    (for **both** modes), not added again in the final write.
...
$$
\Delta\theta = \text{wrap}\!\left(\delta_j - \theta\right)
$$
...
$$
\Delta\theta = \text{wrap}\!\left(\varphi_i + \delta_j - \theta\right)
$$
...
| `osc_indices` | `uint32[2, N_joints]` | Oscillator index pairs per joint (only `[0]`/`osc_i_0` is used for phase; `[1]`/`osc_i_1` is asserted to be out of range) |
| `threshold` | `float` | Amplitude threshold for walk/swim switching (Cython class default `0`; `AmphibiousController` passes `1e-2`) |

!!! warning "No `weight` or `offset` parameters"
    `PositionPhaseCy.__init__` only accepts `state`, `osc_indices`, and `threshold`
...
self.network2joints['position_phase'] = PositionPhaseCy(
    ...
    threshold=1e-2,      # Amplitude below this → swimming mode (limb holds CPG offset)
)
```
