...
This document details the cross-module dependencies and execution loop of
the FARMS framework, at the level of class/inheritance diagrams. For a
higher-level narrative overview first, see [System
...
!!! note "`animat_i` convention"
    `animat_i` is not declared on the abstract `AnimatExtension` base class itself; it is set as an instance attribute by concrete subclasses (e.g. `AnimatController.__init__` sets `self.animat_i`, and `SwimmingExtension.__init__` likewise sets `self.animat_i`). It is shown on `AnimatController` in the diagram above rather than on `AnimatExtension`.
...
classDiagram
    ...
    class AnimatExtension {
        <<ABC>>
        +from_options(cls, config, experiment_options, animat_i, animat_data, animat_options)
    }
...
sequenceDiagram
    ...
    loop Every Iteration
        loop cb_sub_steps times
            ...
            Env->>Task: task.before_step(action, physics)
